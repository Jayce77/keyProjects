...
public class UserIOConsoleImpl implements UserIO{
    ...
    private BigDecimal validBigDecimal, bigDecimalMin, bigDecimalMax;
    Scanner sc;
   ...
   @Override
    ...
    public BigDecimal readBigDecimal(String prompt) {
        ...
        while(!isValid){
           print(prompt);
            if (sc.hasNextBigDecimal()  == true) {
                isValid = true;
                do {
                    strAnswer = sc.nextLine();
                    strAnswer = strAnswer.replaceAll("[\t+,+]","");
                } while ("".equals(strAnswer));
                validBigDecimal = new BigDecimal(strAnswer);
            } else {
               do {
                    strAnswer = sc.nextLine();
                    strAnswer = strAnswer.replaceAll("\t+","");
    ...
    public BigDecimal readBigDecimal(String prompt, String min, String max) {
        bigDecimalMin = new BigDecimal(min);
        bigDecimalMax = new BigDecimal(max);
        ...
        while(!isValid){
           print(prompt);
            if (sc.hasNextBigDecimal()  == true) {
             ...
             if (validBigDecimal.compareTo(bigDecimalMin) == -1  
                     || validBigDecimal.compareTo(bigDecimalMax) == 1) {
                System.out.println("Can't you count??\nTry that again!");
                inBounds = false; 
            } else {
                inBounds = true;
